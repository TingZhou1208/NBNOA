function [species,guide,sub_nums,meandis,slbest,sgbest] = NBNC1(costX,X,popsize,g,max_g,n_var)
%The NBNC algorithm is used to formulate species
 alpha = 5;                       %scale factor for species
% alpha = 1.0 + 0.8*((fitcount + Particle_Number)/Max_FES) ;
% alpha = 1.2+g./max_g;
%  alpha=ceil(g/max_g.*4)+1;   
...
for i=1:popsize
    if X(i,end) ==0
        guideIdx(i,:) = sgbest(i,:); %%后期多
    else
        guideIdx(i,:)= slbest(i,1:2); %%前期多
    end
end
guide = x(guideIdx(:,2),:);
end