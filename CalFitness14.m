function [D_Dec,D_Obj,Fitness] = CalFitness14(PopObj,PopCon,PopDec)
...
for i = 1 : N-1
    for j = i+1 : N
        if (CV(i)<= epsilon  && CV(j)<= epsilon ) || CV(i)==CV(j) %%两个个体都小于一个阈值为可行解 通过比较目标函数值
...
% %% 方案3
% %% 改进的epsilon约束处理结束====PPS
% %%设置阈值
% N = size(PopObj,1);
% Tc               = 0.25 * ceil((gen*Max_Gen)/N);
% epsilon_k        = 0;
% epsilon_0        = 0;
% cp               = 2;
% alpha            = 0.95;
